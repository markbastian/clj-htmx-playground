...
(defmethod event-api/process-event [:htmx :user-left-room]
  [{:keys [clients db]}
   {:keys [username room-name]}]
  (htmx/broadcast-leave-room clients db username room-name))

(defmethod event-api/process-event [:htmx :user-entered-room]
  [{:keys [clients db]}
   {:keys [username room-name]}]
...
(defmethod event-api/process-event [:htmx :room-created]
  [{:keys [clients db]} _]
  (htmx/broadcast-update-room-list clients db))

(defmethod event-api/process-event [:htmx :room-deleted]
  [{:keys [clients db]} _]
  (htmx/broadcast-update-room-list clients db))

(defmethod event-api/process-event [:htmx :user-joined-chat]
  [{:keys [clients db]} _]
  (htmx/broadcast-update-user-list clients db))

(defmethod event-api/process-event [:htmx :user-left-chat]
  [{:keys [clients db]} _]
  (htmx/broadcast-update-user-list clients db))

(defmethod event-api/process-event [:htmx :chat-message-created]
  [{:keys [clients db]}
